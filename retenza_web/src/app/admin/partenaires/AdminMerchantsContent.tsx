          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bricolage font-bold shrink-0 text-sm">
                          {c.name?.[0]?.toUpperCase() || 'C'}
                        </div>
                        <div>
                          <p className="font-semibold text-[#1B100C] text-sm">{c.name}</p>
                          <p className="text-xs text-[#9C8B82]">{c.contact?.city || 'N/A'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-[#1B100C]">
                      {c.category || 'Non spécifié'}
                    </td>
                    <td className="px-6 py-4">
            ...
            {filteredCommerces.length === 0 && (
              <div className="py-8 text-center text-gray-500 bg-gray-50">
                <Store className="w-8 h-8 mx-auto text-gray-300 mb-2" />
                <p className="text-sm font-medium">Aucun partenaire trouvé</p>
              </div>
            )}
          </div>
        </>
