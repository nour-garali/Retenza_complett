...
export default function ClientChatbotPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/chatbot/client-status?email=${encodeURIComponent(email)}&commerce_id=${encodeURIComponent(selectedCommerceId)}`)
      .then((r) => r.json())
      .then((data) => {
    ...
    const loadExistingConversation = async () => {
      try {
        const ticketRes = await fetch(
          `http://localhost:5000/api/chatbot/support-tickets?email=${encodeURIComponent(email)}&commerce_id=${encodeURIComponent(selectedCommerceId)}`
        );
        ...
        const convRes = await fetch(
          `http://localhost:5000/api/chatbot/conversation/${encodeURIComponent(email)}?commerce_id=${encodeURIComponent(selectedCommerceId)}&session_id=${encodeURIComponent(sid)}`
        );
  ...
  useEffect(() => {
    ...
    const pollForSupportMessages = async () => {
      try {
        ...
        const r = await fetch(
          `http://localhost:5000/api/chatbot/conversation/${encodeURIComponent(email)}?commerce_id=${encodeURIComponent(selectedCommerceId)}&session_id=${encodeURIComponent(sessionId)}`
        );
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/chatbot/verify-session", {
        method: "POST",
              ...
              if (trimmed.startsWith("data: ")) {
                try {
                  ...
                  if (data.type === "token" && data.content) {
                    ...
                    fetch("http://localhost:5000/api/chatbot/save-warning", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
                        email,
  ...
  const setEscalateTrigger = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/chatbot/support-tickets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: email,
  ...
  const handleSendSupportMessage = async (e?: React.FormEvent) => {
    ...
    try {
      await fetch("http://localhost:5000/api/chatbot/support-message", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: email,
  ...
  const handleFeedback = async (msgId: string, text: string, type: "like" | "dislike") => {
    ...
    try {
      await fetch("http://localhost:5000/api/chatbot/message-feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: email,
                  ...
                  <div className="pt-1.5 border-t border-amber-200/60 flex items-center gap-1.5 flex-wrap">
                    ...
                    {(() => {
                      const botCount = supportMessages.filter((m) => {
                        ...
                        return ch === "bot" || (ch as string) === "bot_context";
                      }).length;
            ...
            <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5 bg-[#F5F4F2]">
              {(() => {
                ...
                const renderSingleMessage = (msg: Message) => {
                  const isUser = msg.role === "user" || (msg.role as string) === "client_support";
                  const isSupport = msg.role === "support";
                  const isBotContext = (msg.channel as string) === "bot_context";
