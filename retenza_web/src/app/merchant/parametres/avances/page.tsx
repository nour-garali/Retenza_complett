...
type TabType = "all" | "boutique_defaut" | "smart_automation" | "heures_creuses" | "anniversaire" | "absence" | "securite" | "cross_sell" | "costs";
...
export default function ParametresAvancesPage() {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const resGen = await fetch("http://localhost:5000/api/commerces/settings", {
        method: "POST",
      ...
      const resLt = await fetch("http://localhost:5000/api/low-traffic/settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#F7F5F2]">
      ...
      <div className="px-6 md:px-8 mt-0">
        <div className="flex flex-nowrap border-b border-[#EEE5DF] gap-3 sm:gap-4 lg:gap-5 max-w-7xl mx-auto overflow-x-auto no-scrollbar [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-0">
          {[
            ...
            return (
              <button
                ...
                className={`pb-2.5 pt-1 px-1 sm:px-1.5 text-[13px] font-bold border-b-2 -mb-px transition-all cursor-pointer whitespace-nowrap shrink-0 flex items-center gap-1.5 ${
                  isActive
