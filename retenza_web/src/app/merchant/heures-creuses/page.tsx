...
export default function HeuresCreuses() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await Promise.all([
        fetch(`http://localhost:5000/api/low-traffic/settings?commerce_id=${targetId}`, { cache: "no-store" }),
        fetch(`http://localhost:5000/api/low-traffic/snapshot?commerce_id=${targetId}`, { cache: "no-store" }),
        fetch(`http://localhost:5000/api/low-traffic/history?commerce_id=${targetId}`, { cache: "no-store" })
      ]);
  ...
  const save = async () => {
    ...
    try {
      const activeCommerceId = commerces.find(c => c.id === "commerce_local_1")?.id || "commerce_local_1";
      const targetId = id === "__all__" ? activeCommerceId : id;
      const r = await fetch('http://localhost:5000/api/low-traffic/settings', {
        method: 'POST',
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      <PageHeader
        ...
        subtitle="Affluence et offres dynamiques ciblées."
      >
        <select
          value={id}
