...
function CampaignsContent() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const histRes = await fetch(`http://localhost:5000/api/campaigns/history/__all__?commerce_id=${encodeURIComponent(selectedCommerce)}`);
  ...
  const checkAutomationStatus = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/campaigns/automation-status");
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/commerces/settings?commerce_id=${encodeURIComponent(cid)}`, {
      cache: "no-store",
      credentials: "include",
    })
  ...
  return (
    ...
    <div className="flex-1 flex flex-col min-h-screen">
      <PageHeader
        ...
        subtitle="Statistiques et automatisation de vos envois."
      >
        {/* Boutique Selector */}
        <select
        ...
        <a
          href={`http://localhost:5000/api/export/campaigns?commerce_id=${selectedCommerce}`}
          download
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0 items-start">
        ...
        <div className="lg:col-span-2 bg-white border border-[#EEE5DF] rounded-2xl p-6 shadow-sm flex flex-col gap-4 h-fit">
        ...
        <div className="bg-white border border-[#EEE5DF] rounded-2xl p-6 shadow-sm flex flex-col gap-4 h-fit">
          ...
          {/* Mini-historique des dernières exécutions IA */}
          {(() => {
            const isAiRun = (item: CampaignHistoryItem) =>
              (item.status && item.status.includes("auto")) ||
              (!!item.category && item.category !== "group" && item.category !== "unknown" && item.category !== "manual");

            // Group by date of AI runs (each "run" = all emails sent that day by IA)
            const aiItems = history.filter(isAiRun);
