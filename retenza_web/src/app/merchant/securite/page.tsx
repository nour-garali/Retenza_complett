...
export default function SecurityAdminPage() {
  ...
  const fetchAlerts = async (targetId: string = commerceId) => {
    try {
      const res = await fetch(`http://localhost:5000/api/security/fraud-alerts?commerce_id=${targetId}`, {
        cache: "no-store",
      });
  ...
  const fetchTrustScores = async (targetId: string = commerceId) => {
    try {
      const res = await fetch(`http://localhost:5000/api/security/trust-scores?commerce_id=${targetId}`, {
        cache: "no-store",
      });
      const json = await res.json();
  ...
  const handleUnblockFraud = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/security/unblock-client", {
        method: "POST",
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      <PageHeader
        ...
        subtitle="Surveillance des anomalies et prévention des fraudes."
      >
        <button
